...
Public Sub MakeFlightLogTable()
    ...
    ' 1. 기존 테이블들 삭제 (출력용 및 임시)
    DoCmd.SetWarnings False
    On Error Resume Next
    DoCmd.DeleteObject acTable, "비행기록부_출력용"
    DoCmd.DeleteObject acTable, "FlightLog_Temp"
    Err.Clear
    On Error GoTo ErrorHandler

    ' 2. [1단계] 임시 테이블 생성 (비행기록부 + 기체정보)
    ' 가장 문제가 되었던 조인을 분리하여 안정성을 확보합니다.
    sqlTemp = "SELECT " & _
              "F.ID, F.교육생ID, F.교관ID, F.기체ID, F.교육일, F.교육시작시간, F.교육종료시간, F.교육시간, " & _
              "A.기종, A.종류, A.형식, A.신고번호, A.자체중량, A.최대이륙중량, A.[최근안전성인증일] " & _
              "INTO FlightLog_Temp " & _
              "FROM [비행기록부] AS F INNER JOIN [기체정보] AS A ON F.기체ID = A.ID"
    DoCmd.RunSQL sqlTemp

    ' 3. [2단계] 최종 출력용 테이블 생성 (임시 테이블 + 교육생명단 + 직원명단)
    ' 이제 안정적으로 생성된 임시 테이블을 기반으로 나머지 조인을 수행합니다.
    sql = "SELECT " & _
          "Temp.기종, Temp.종류, Temp.형식, Temp.신고번호, Temp.자체중량, Temp.최대이륙중량, Temp.[최근안전성인증일], " & _
          "T.이름 AS 교육생성명, T.ID AS 교육생내부ID, Temp.교육생ID, " & _
          "Temp.교육일 AS 일자, '전남 나주' AS 비행장소, " & _
          "Temp.교육시작시간 AS 이륙시간, Temp.교육종료시간 AS 착륙시간, " & _
          "Temp.교육시간 AS 비행시간_분, Round(Temp.교육시간 / 60, 1) AS 비행시간_시간, " & _
          "S.성명 AS 교관성명, S.자격번호 AS 교관자격번호, " & _
          "(SELECT Count(*) FROM [비행기록부] AS F2 WHERE F2.교육생ID = Temp.교육생ID AND (F2.교육일 < Temp.교육일 OR (F2.교육일 = Temp.교육일 AND F2.교육시작시간 <= Temp.교육시작시간))) AS 비행순서 " & _
          "INTO [비행기록부_출력용] " & _
          "FROM ((FlightLog_Temp AS Temp " & _
          "INNER JOIN [0-교육생명단] AS T ON Temp.교육생ID = T.출퇴근ID) " & _
          "LEFT JOIN [직원명단] AS S ON Temp.교관ID = S.직원ID)"
    DoCmd.RunSQL sql
    ...
    ' 5. [4단계] 추가된 필드 값 계산 및 업데이트
    ' 5-1. 비행목적, 시간 등 간단한 계산 업데이트 (한 번에 실행)
    ...
    ' 5-2. 교육과정 정보 업데이트 (복잡한 로직은 VBA Recordset으로 순회하며 처리)
    ...
    MsgBox "출력용 테이블 생성 완료: 비행기록부_출력용", vbInformation
    Exit Sub

ErrorHandler:
    DoCmd.SetWarnings True
    On Error Resume Next ' 임시 테이블 삭제 시도
    DoCmd.DeleteObject acTable, "FlightLog_Temp"
    On Error GoTo 0
